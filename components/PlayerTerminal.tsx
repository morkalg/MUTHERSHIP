...
import type { TerminalEntry, ColorTheme, ShipSystem } from '../types';
...
const PlayerTerminal: React.FC<PlayerTerminalProps> = ({ history, isLoading, onSendCommand, colorTheme, currentUserRole, shipSystems }) => {
          ...
          <div key={index} className="mb-2 whitespace-pre-wrap break-words">
            {entry.type === 'command' ? (
              ...
              <div>
                ...
                {entry.visual && (
                  <SystemVisualizer
                    systemName={entry.visual}
                    colorTheme={colorTheme}
                    system={shipSystems?.find(s => s.name === entry.visual)}
                  />
                )}
              </div>
            )}
